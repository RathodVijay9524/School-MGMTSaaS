...
public class UserController {
    ...
    * */
    ...
    public ResponseEntity<PageableResponse<UserResponse>> getAllActiveUsers(
        ...
        Sort sort = sortDir.equalsIgnoreCase("desc") ? Sort.by(sortBy).descending() : Sort.by(sortBy).ascending();
        Pageable pageable = PageRequest.of(pageNumber, pageSize, sort);
        PageableResponse<UserResponse> users = userService.getAllActiveUsers(pageable);
    ...
    public ResponseEntity<PageableResponse<UserResponse>> getAllDeletedUsers(
        ...
        Sort sort = sortDir.equalsIgnoreCase("desc") ? Sort.by(sortBy).descending() : Sort.by(sortBy).ascending();
        Pageable pageable = PageRequest.of(pageNumber, pageSize, sort);
        PageableResponse<UserResponse> users = userService.getAllDeletedUsers(pageable);
        return new ResponseEntity<>(users, HttpStatus.OK);
    }
    ...
    public ResponseEntity<ImageResponse> uploadUserImage(@RequestParam("userImage") MultipartFile image) {
        try {
            ...
            userResponse.setRoles(activeRoles.stream()
                    .map(Role::getName)
                    .collect(Collectors.toSet()));
            ...
            UserRequest userRequest = UserRequest.builder()
                    ...
                    .deletedOn(userResponse.getDeletedOn() != null ? 
                        userResponse.getDeletedOn().toInstant().atZone(java.time.ZoneId.systemDefault()).toLocalDateTime() : 
                        null)
